...
// ══════════════════════════════════════════
// RESTORE TODAY'S SESSION from RTDB
// ══════════════════════════════════════════
async function loadFirebaseState() {
    try {
        ...
        snap.forEach(child => {
            const d = child.val();
            if (d.collection_date === today) todayMap[String(d.vendor_id)] = d;
        });
        ...
        console.log(`[RTDB] Restored ${Object.keys(todayMap).length} vendor states for ${today}`);
    } catch (e) { console.error('loadFirebaseState:', e.message); }
}

// ══════════════════════════════════════════
// RTDB WRITE
// Key: vendor_id_YYYY-MM-DD → one record per vendor per day
// ══════════════════════════════════════════
async function syncToFirebase(vendor) {
    ...
    try {
        ...
        console.error(`syncToFirebase vendor ${vendor.vendor_id}:`, e.message);
    }
}
...
async function upsertToSupabase(vendor, paidAmount) {
    ...
    // Base payload — always valid fields
    const payload = {
    ...
    // Only include collection_fee_id if we have a valid one (avoids FK violation)
    if (fee.id) payload.collection_fee_id = fee.id;

    const { error } = await supabase
        .from('tax_dscrpt_summary')
        .upsert(payload, { onConflict: 'vendor_id,collection_date' });
